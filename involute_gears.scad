...
// 5/25/11 - modification by jag to add dedendum_adjustment and addendum_adjustment to allow for more clearance with internal gears
// 3/29/19 - updated to remove warnings in newer OpenSCAD -  https://hackaday.io/daren
	...
	flat=false)
{
	...
	// Addendum: Radial distance from pitch circle to outside circle.
	addendum = addendum_adjustment/pitch_diametrial;

	//Outer Circle
	outer_radius = pitch_radius+addendum;

	// Dedendum: Radial distance from pitch circle to root diameter
	dedendum = dedendum_adjustment/pitch_diametrial + clearance;
...
module linear_exturde_flat_option(flat =false, height = 10, center = false, convexity = 2, twist = 0)
{
	if(flat==false)
	{
		linear_extrude(height = height, center = center, convexity = convexity, twist= twist) children();
	}
	else
	{
		children();
